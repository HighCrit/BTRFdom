...
def get_mtl_block(tmlFile, rootBlock, material_info):
    ...
    # Read self_illumi from node tree using named input (version-safe)
    self_illumi = 0.0
    if material_info.material.use_nodes and material_info.material.node_tree:
        bsdf = material_info.material.node_tree.nodes.get("Principled BSDF")
        if bsdf and 'Emission Strength' in bsdf.inputs:
            self_illumi = bsdf.inputs['Emission Strength'].default_value
